"""
Main LightKey MIDI control script
"""
...
from utils.midi_sender import get_midi_client
from config import LIGHTKEY_FIXTURE_MAPPING

class LightKeyMIDIPlayer:
    """Real-time LightKey MIDI playback"""
    ...
    def __init__(self, cues_file: str):
        ...
        """
        self.midi = get_midi_client()
        self.cues_file = cues_file
        self.cues = self._load_cues()
        
        print(f"\n{'='*70}")
        print(f"🎹 LIGHTKEY MIDI PLAYBACK ENGINE")
        print(f"{'='*70}\n")
        print(f"📁 Cues file: {cues_file}")
        print(f"📊 Total cues: {len(self.cues)}")
        print(f"⏱️  Duration: {self._get_duration():.1f}s")
        print(f"🎹 MIDI Port: {self.midi.port_name}")
        print(f"🎛️  MIDI Enabled: {self.midi.enabled}")
        print(f"{'='*70}\n")
        
        if not self.midi.enabled:
            print("⚠️  WARNING: MIDI not connected!")
            print("   Run: python setup_midi.py\n")
    ...
    def play(self):
        ...
        if not self.midi.enabled:
            print("❌ MIDI not connected. Cannot play.")
            return
    ...
    def _execute_cue(self, cue: dict, cue_num: int):
        ...
        for fixture_cue in cue.get("cues", []):
            ...
            if lightkey_num:
                self.midi.set_fixture_dmx_channels(lightkey_num, dmx_channels)
                ...
                print(f"  → {fixture_id} (MIDI Fixture#{lightkey_num}): RGB({r},{g},{b}) @ {intensity}/255")
...
def main():
    """Main entry point"""
    if len(sys.argv) < 2:
        print("\n⚠️  Usage: python lightkey_midi_control.py <cues_file>\n")
        print("Example:")
        print("  python lightkey_midi_control.py data/lighting_cues/Script-1_cues.json\n")
        sys.exit(1)
    ...
    # Create player
    player = LightKeyMIDIPlayer(cues_file)
