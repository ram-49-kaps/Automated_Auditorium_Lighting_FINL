...
|--------|-------------|--------|
| `LightingInstruction` | Phase 5, Phase 7 | Pydantic model / dict |

Baseline: 2 groups per scene (`front_wash`, `back_light`).
...
|------|--------|-------------|
| Rule-based (baseline) | ✅ Active | Deterministic, uses `retrieve_palette` |
...
- Rule-based uses only 2/5 groups (coverage = 0.4)
